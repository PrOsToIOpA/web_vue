...
<div class="container" style="max-width: 600px;justify-content: center;padding: 100px 0;" @submit.prevent="submitHandler">
  ...
  <form class="form-horizontal">
    ...
    <div class="form-group">
      <label class="control-label col-xs-3" for="inputname">Введите имя:</label>
      <div class="col-xs-9">
        <input type="text" class="form-control" id="inputname" placeholder="Введите имя" v-model.trim="name" :class="$v.name.$dirty && !$v.name.required">
        <small id="passwordHelp" class="form-text text-muted" v-if="$v.name.$dirty && !$v.name.required">Поле не должно быть пустым</small>
      </div>
    </div>
    <div class="form-group">
      <div class="col-xs-offset-2 col-xs-10">
        <div class="checkbox">
          <label><input type="checkbox" v-model="agree"> С правиламии согласен</label>
        </div>
      </div>
    </div>
    <div class="form-group">
      <div class="col-xs-offset-2 col-xs-12">
        <button type="submit" class="btn btn-secondary" style="float:left;">Зарегестрироваться</button>
        <router-link to="/login" class="btn btn-primary" style="float:right;">Войти</router-link>
      </div>
    </div>
  </form>
  </div>
